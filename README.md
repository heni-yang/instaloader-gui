...
- **Anti-Detection 4단계 시스템**: 🚫 OFF → 🚀 FAST → ⚖️ ON(권장) → 🛡️ SAFE
- **지능형 Rate Controller**: 모드별 최적화된 요청 제어
...
### 🔐 보안 기능
- **Fernet 암호화**: 계정 비밀번호 안전 저장
- **머신 지문**: 기기별 고유 암호화 키 생성
- **보안 로깅**: 민감 정보 자동 마스킹 (사용자명, 비밀번호)
- **세션 보호**: 암호화된 세션 파일 관리
...
- **🚫 OFF (즉시 실행)**: 대기시간 없음 (위험도: 매우 높음) - 소량 프로필, 빠른 테스트용
- **🚀 FAST (고속)**: 고속 크롤링 (위험도: 높음) - 빠른 크롤링, 중소량 프로필
- **⚖️ ON (권장)**: 균형잡힌 속도와 안전성 - 일반적인 사용, 중간 규모
- **🛡️ SAFE (대량/장시간)**: 안전한 대량 다운로드 - 대량 프로필, 장시간 작업
...
#### 모드별 특성
- **🚫 OFF**: 즉시 실행으로 최고 속도, 차단 위험 매우 높음
- **🚀 FAST**: 고속 모드로 빠른 크롤링, 중소량 프로필에 적합
- **⚖️ ON**: 권장 모드로 속도와 안전성의 균형, 일반 사용
- **🛡️ SAFE**: 안전 모드로 대량/장시간 작업에 최적화

#### 자동 설정 마이그레이션
- 기존 요청 간 대기시간 설정이 자동으로 적절한 모드로 변환
- 신규 사용자는 기본적으로 ON (권장) 모드로 시작
